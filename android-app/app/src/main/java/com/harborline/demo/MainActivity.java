...
public class MainActivity extends Activity {
    ...
    private void showDashboard(DatabaseHelper.Account account) {
        ...
        add(page, text("AVAILABLE BALANCE", 11, TEAL), 36); add(page, text(format(account, account.balance), 40, DEEP), 70); add(page, text("ACCOUNT PROFILE", 11, TEAL), 36); showAccount(account); add(page, text("ACCOUNT REVIEW", 11, TEAL), 36); add(page, text("Submit a review request and follow its processing status.", 14, MUTED), 48); reviewButton = button("Request account review", DEEP); add(page, reviewButton, 58); reviewStatus = text("", 13, TEAL); add(page, reviewStatus, 82); refreshReviewStatus(account); reviewButton.setOnClickListener(v -> submitReview(account)); rotating = true; rotationHandler.removeCallbacks(rotation); rotationHandler.postDelayed(rotation, 10000);
    }

    private void showAccount(DatabaseHelper.Account account) { if (dashboard == null) return; TextView profile = (TextView) dashboard.findViewWithTag("profile"); if (profile == null) { profile = text("", 15, INK); profile.setTag("profile"); dashboard.addView(profile, new LinearLayout.LayoutParams(-1, -2)); } profile.setText("Name: " + account.name + "\nEmail: " + account.email + "\nLocation: " + account.city + ", " + account.country + "\nDate of birth: " + account.dob + "\nAccount number: " + account.accountNumber + "\nDeposited: " + format(account, account.deposited) + "\nPrevious withdrawals: " + format(account, account.withdrawn) + "\nStatus: Active demo profile\n\nRecent activity\n• Opening deposit / account deposit\n• Previous withdrawal summary\n\nLinked card: 5333 17•• •••• 3046\nIBAN: IT21 **** **** **** 79253\nBIC/SWIFT: PPAYITR1XXX"); if (reviewStatus != null) refreshReviewStatus(account); }
    private void submitReview(DatabaseHelper.Account account) { long requested = System.currentTimeMillis(); long expected = requested + (3L * 24 * 60 * 60 * 1000); reviewStore.edit().putLong("requested_" + account.id, requested).putLong("expected_" + account.id, expected).apply(); refreshReviewStatus(account); Toast.makeText(this, "Review submitted for processing. Decision expected after 3 days.", Toast.LENGTH_LONG).show(); }
    private void refreshReviewStatus(DatabaseHelper.Account account) { long requested = reviewStore.getLong("requested_" + account.id, 0); if (requested == 0) { reviewStatus.setText("No review request submitted"); reviewButton.setEnabled(true); reviewButton.setText("Request account review"); } else { java.text.DateFormat dateFormat = java.text.DateFormat.getDateInstance(java.text.DateFormat.MEDIUM, Locale.US); reviewStatus.setText("Processing\nRequested " + dateFormat.format(requested) + ". Review and approval decision expected after " + dateFormat.format(reviewStore.getLong("expected_" + account.id, requested)) + "."); reviewButton.setEnabled(false); reviewButton.setText("Review request processing"); } }
